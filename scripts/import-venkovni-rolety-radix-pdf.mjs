...
const DEFAULT_PDF =
  ...
  "C:/Users/ludvi/Desktop/Qapieshop/Katalogy/10_CENIK_venkovni_rolety.pdf";
    ...
    if (r.rows[0]) {
      ...
      await client.query(
        `UPDATE "Product" SET category=$2, price=$3, img=$4, "desc"=$5, badge=$6,
          ...
          max_area_m2 = NULL
         WHERE id = $1`,
        [id, "Venkovní stínění", minPrice, IMG, desc, "Na míru", "venkovni_roleta_radix"]
      );
    } else {
      const ins = await client.query(
        `INSERT INTO "Product" (title, category, price, badge, img, "desc",
         ...
         VALUES ($1, $2, $3, $4, $5, $6, 4.9, 0, 'matrix_cell', $7, 700, 3000, 600, 3400) RETURNING id`,
        [PRODUCT_TITLE, "Venkovní stínění", minPrice, "Na míru", IMG, desc, "venkovni_roleta_radix"]
      );
      id = ins.rows[0].id;
    }
