...
const DEFAULT_XLSX =
  ...
  "C:/Users/ludvi/Desktop/Qapieshop/Katalogy/05_CENIK_rolety_DN_Collete_PLUS_DPH.xlsx";
  ...
  if (r.rows[0]) {
    ...
    await client.query(
      `UPDATE "Product" SET category=$2, price=$3, img=$4, "desc"=$5, badge=$6,
       ...
       WHERE id = $1`,
      [id, "Interiérové stínění", minPrice, IMG, desc, "Na míru"]
    );
  } else {
    const ins = await client.query(
      `INSERT INTO "Product" (title, category, price, badge, img, "desc", supplier_markup_percent, commission_percent, price_mode)
       VALUES ($1, $2, $3, $4, $5, $6, 4.9, 0, 'matrix_cell') RETURNING id`,
      [title, "Interiérové stínění", minPrice, "Na míru", IMG, desc]
    );
    id = ins.rows[0].id;
  }
